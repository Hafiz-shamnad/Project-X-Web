...
export default function Leaderboard({
  ...
  const LeaderboardRow = useCallback(
    ({ item, idx }: { item: TeamData; idx: number }) => (
      <div
        key={item.id ?? idx}
      ...
      >
        ...
        <div className="text-right">
          <div className="text-3xl font-black text-blue-300">{item.score}</div>
          ...
          <LineChart margin={{ top: 10, right: 30, left: -10, bottom: 10 }}>
            ...
            {Object.entries(timelineData).map(([label, data], idx) => (
              <Line
                ...
                strokeWidth={2}
                isAnimationActive={false} // huge perf upgrade
                dot={false} // another huge perf upgrade
                activeDot={{ r: 5 }}
              />
            ))}
          </LineChart>
      ...
      <div className="bg-slate-950/70 backdrop-blur-2xl border border-blue-500/30 rounded-3xl overflow-hidden shadow-xl shadow-blue-500/10">
        ...
        {leaderboard.length === 0 ? (
          ...
          leaderboard
            .slice(0, 10)
            .map((item, idx) => (
              <LeaderboardRow key={item.id ?? idx} item={item} idx={idx} />
            ))
        )}
      </div>
    </div>
