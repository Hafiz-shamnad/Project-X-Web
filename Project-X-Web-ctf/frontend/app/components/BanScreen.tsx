...
export default function BanScreen({ type, timer }: BanScreenProps) {
  ...
  const handleButtonClick = () => {
    if (isTemp) window.location.reload();
    else window.location.href = "/contact-admin";
  };

  const bgGrid =
    "absolute inset-0 bg-[linear-gradient(rgba(239,68,68,0.03)_1px,transparent_1px),linear-gradient(90deg,rgba(239,68,68,0.03)_1px,transparent_1px)] bg-[size:50px_50px]";
  const glowCircle = "absolute w-96 h-96 rounded-full blur-3xl animate-pulse";
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-950 via-blue-950/20 to-slate-950 font-mono text-red-400 relative overflow-hidden">
      ...
      <div className="fixed inset-0 -z-10">
        ...
        <div className={`${glowCircle} top-1/4 left-1/4 bg-red-500/10`} />
        <div className={`${glowCircle} bottom-1/4 right-1/4 bg-rose-500/10`} />
      </div>
